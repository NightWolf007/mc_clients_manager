defmodule ClientsManager.Client.Handler do
  ...
  def handle_event(message, %{client_id: client_id,
    ...
    case Table.find(:clients, client_id) do
      {:ok, {%{type: type, data: data} = client}} ->
        ...
        handle(msg, client_id, client, redis_client)
        {:ok, state}
      _ ->
        nil
    end
  ...
  # client connected
  defp handle({:connected}, client_id, _client, redis_client) do
    IO.puts("CONNECTED #{client_id}")
    Messenger.connected(redis_client, client_id)
  end

  # client found
  defp handle({:found}, client_id, _client, redis_client) do
    IO.puts("FOUND #{client_id}")
    Messenger.found(redis_client, client_id)
  end

  # client sent message
  defp handle({:message, message}, client_id, client, redis_client) do
    IO.puts("MESSAGE #{client_id} #{message}")
    if !Client.muted?(client) do
      Forwarder.forward_from(client_id, message)
      Messenger.message(redis_client, client_id, message, "SENT")
    else
      Messenger.message(redis_client, client_id, message, "BLOCKED")
    end
  end

  # client leaved dialog
  defp handle({:leaved}, client_id, client, redis_client) do
    IO.puts("LEAVED #{client_id}")
    Messenger.leaved(redis_client, client_id)
    handle({:closed}, client_id, client, redis_client)
  end

  # dialog closed
  defp handle({:closed}, client_id, _client, redis_client) do
    IO.puts("CLOSED #{client_id}")
    Messenger.closed(redis_client, client_id)
  end

  # unknown message
  defp handle({:unknown}, _client_id, _client, _redis_client) do
  end
end
